...
func jsonFormat(pass string) {
	pairs := strings.Split(pass[1:], " ")
...
func isValidPassport(currentPassport string) error {
	currentPassport = currentPassport[1:]
	for _, key := range keys {
		if !strings.Contains(currentPassport, key) {
			// validPassports++
			// fmt.Println(currentPassport, "missing key", key)
			return fmt.Errorf("missing key %s", key)
		}
	}
	// tem todos os campos
	// fmt.Println(currentPassport)
	passaport := strings.Split(currentPassport, " ")
	// fmt.Println("passaport", passaport, len(passaport))
	for _, key := range passaport {

		current := strings.Split(key, ":")
		// fmt.Println("key", current)
		val := current[1]
		switch current[0] {
		// byr (Birth Year) - four digits; at least 1920 and at most 2002.
		case "byr":
			// if len(val) != 4 {
			// 	return fmt.Errorf("%s invalid len %s", current[0], val)
			// }
			year, err := strconv.Atoi(val)
			if err != nil {
				return fmt.Errorf("%s invalid year %s", current[0], val)
			}
			if year < 1920 || year > 2002 {
				return fmt.Errorf("%s invalid year %s", current[0], year)
			}
		// iyr (Issue Year) - four digits; at least 2010 and at most 2020.
		case "iyr":
			// if len(val) != 4 {
			...
			if year < 2010 || year > 2020 {
				return fmt.Errorf("%s invalid year %d", current[0], year)
			}
			// eyr (Expiration Year) - four digits; at least 2020 and at most 2030.
		case "eyr":
			// if len(val) != 4 {
			...
			// If in, the number must be at least 59 and at most 76.
		case "hgt":
			if !strings.Contains(val, "cm") && !strings.Contains(val, "in") {
				return fmt.Errorf("%s missing cm or in %s", current[0], val)
			}
			if strings.Contains(val, "cm") {
				val = strings.Replace(val, "cm", "", -1)
				height, err := strconv.Atoi(val)
				if err != nil {
					return fmt.Errorf("%s height not a number %s", current[0], val)
				}
				if height < 150 || height > 193 {
					return fmt.Errorf("%s invalid cm height %d", current[0], height)
				}
			}
			if strings.Contains(val, "in") {
				...
				// fmt.Println(height < 59, height > 76)
				if height < 59 || height > 76 {
					return fmt.Errorf("%s invalid in height %d", current[0], height)
				}
			}
			// hcl (Hair Color) - a # followed by exactly six characters 0-9 or a-f.
		case "hcl":
			// if !strings.HasPrefix(val, "#") && len(val) != 6 {
			// 	return fmt.Errorf("%s missing # or len invalid %s", current[0], val)
			// }
			if !validHCL.MatchString(val) {
				return fmt.Errorf("%s - %s no match", current[0], val)
			}
			// ecl (Eye Color) - exactly one of: amb blu brn gry grn hzl oth.
		case "ecl":
			// fmt.Println(val, eyes)
			if !strings.Contains(eyes, val) {
				return fmt.Errorf("%s invalid eyes %s", current[0], val)
			}
			// pid (Passport ID) - a nine-digit number, including leading zeroes.
		case "pid":
			// if len(val) != 9 {
			// 	return fmt.Errorf("%s doesn't have 9 digits %d, %s", current[0], len(val), val)
			// }
			if !validPID.MatchString(val) {
				return fmt.Errorf("%s failed match : %s", current[0], val)
			}
			// cid (Country ID) - ignored, missing or not.
		}
	}
	// fmt.Println("valid", currentPassport)
	// return true
	return nil
}

func main() {
	// lines := strings.Split(string(invalid), "\n")
	...
	for _, line := range lines {
		if line == "" {
			//validate passaport
			// fmt.Println("is valid ", currentPassport)

			if err := isValidPassport(currentPassport); err == nil {
				jsonFormat(currentPassport)
				validPassports++
			} else {
				// fmt.Println("invalid", currentPassport, err)
			}

			currentPassport = ""
		} else {
			// join to current passaport
	...
	fmt.Println("are valid", validPassports)
}
