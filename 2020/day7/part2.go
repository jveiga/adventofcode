...
func main() {
	...
	fmt.Println(rules)
	total := CalculateSum("shiny gold")

	fmt.Println(total)
}

func CalculateSum(color string) int {
	fmt.Println(color)
	keys := rules[color]
	if len(keys) == 0 {
		return 0
	...
	for _, key := range keys {
		total += key.Num * CalculateSum(key.Color)
		// fmt.Println(key.Num, "+", key.Num, "*", sum)
		// total += key.Num + key.Num*CalculateSum(key.Color)
